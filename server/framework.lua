...
function Framework.GetName(player)
    ...
    if Framework.name=='esx' then
        local first=(player.get and player.get('firstName')) or player.firstName or 'Unknown'; local last=(player.get and player.get('lastName')) or player.lastName or ''
        return first..' '..last
    end
    local c=player.PlayerData.charinfo or {}; return (c.firstname or '')..' '..(c.lastname or '')
end
...
function Framework.RemoveMoney(player,account,amount,reason)
    account=account=='cash' and 'cash' or 'bank'
    if Framework.name=='esx' then return player.removeAccountMoney(account,amount,reason or 'dmv-registration')~=false end
    return player.Functions and player.Functions.RemoveMoney and player.Functions.RemoveMoney(account,amount,reason or 'dmv-registration') or false
end

function Framework.AddMoney(player,account,amount,reason)
    account=account=='cash' and 'cash' or 'bank'
    if Framework.name=='esx' then return player.addAccountMoney(account,amount,reason or 'dmv-refund') end
    return player.Functions and player.Functions.AddMoney and player.Functions.AddMoney(account,amount,reason or 'dmv-refund') or false
end
...
function Framework.AddItem(source,item,metadata)
    ...
    local p=Framework.GetPlayer(source); return p and p.Functions and p.Functions.AddItem and p.Functions.AddItem(item,1,false,metadata,'st_dmvsystem') or false
end
...
function Framework.RemoveItem(source,item,metadata)
    ...
    if GetResourceState('qb-inventory')=='started' then
        return exports['qb-inventory']:RemoveItem(source,item,1,nil,metadata,'st_dmvsystem')
    end
    local p=Framework.GetPlayer(source); return p and p.Functions and p.Functions.RemoveItem and p.Functions.RemoveItem(item,1,false,metadata,'st_dmvsystem') or false
end

function Framework.HasAdmin(source)
    if source==0 or IsPlayerAceAllowed(source,Config.AdminAce) then return true end
    local p=Framework.GetPlayer(source); if not p then return false end
    if Framework.name=='esx' then
        local group=p.getGroup and p.getGroup() or ''
        for _,allowed in ipairs(Config.AdminGroups) do if group==allowed then return true end end
    else
        local group=p.PlayerData.permission or (p.PlayerData.job and p.PlayerData.job.name)
        for _,allowed in ipairs(Config.AdminGroups) do if group==allowed then return true end end
    end
    return false
end
