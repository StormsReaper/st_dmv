...
RegisterNetEvent('st_dmv:client:showDocument', function(item)
    ...
    SendNUIMessage({ action = 'document', data = data }); SetNuiFocus(true, true); menuOpen = true
end)

exports('UseDocument', function(data) TriggerEvent('st_dmv:client:showDocument', data) end)

RegisterNetEvent('st_dmv:client:setPlate', function(netId, plate)
    local vehicle = NetworkGetEntityFromNetworkId(tonumber(netId) or 0)
    if vehicle ~= 0 and DoesEntityExist(vehicle) then SetVehicleNumberPlateText(vehicle, plate) end
end)
...
RegisterNetEvent('st_dmv:client:installPlate', function()
    local vehicle = getNearbyVehicle(); if not vehicle then return ClientFramework.Notify('No vehicle nearby.', 'error') end
    ...
    if usingOx then
        ...
        for _, entry in pairs(playerData.items or {}) do
            if entry and entry.name == 'dmv_license_plate' then TriggerServerEvent('st_dmv:server:installPlate', netId, entry.info or entry.metadata or {}); return end
        end
    end
...
RegisterNetEvent('st_dmv:client:locationAdded', function(location)
    dmvLocations[#dmvLocations + 1] = { id=location.id, label=location.label, coords=vector3(location.coords.x,location.coords.y,location.coords.z), radius=1.8 }; SetupDMVTargets()
end)
...
CreateThread(function()
    math.randomseed(GetGameTimer()); TriggerServerEvent('st_dmv:server:getLocations')
    while true do
        local sleep=1000; local ped=PlayerPedId(); local coords=GetEntityCoords(ped)
        for _,loc in ipairs(dmvLocations) do
            local dist=#(coords-loc.coords)
            if dist<20.0 and Config.Target=='none' then
                sleep=0; DrawMarker(Config.Marker.type,loc.coords.x,loc.coords.y,loc.coords.z+0.15,0,0,0,0,0,0,Config.Marker.scale.x,Config.Marker.scale.y,Config.Marker.scale.z,255,255,255,150,false,true,2,false,nil,nil,false)
                if dist<2.0 then
