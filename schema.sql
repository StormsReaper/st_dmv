...
CREATE TABLE IF NOT EXISTS dmv_licenses (
  ...
  PRIMARY KEY (id),
  UNIQUE KEY uq_dmv_license_number (license_number),
  UNIQUE KEY uq_dmv_driver_license (citizenid, license_type),
  KEY idx_dmv_license_citizen (citizenid)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS dmv_violations (
  ...
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  KEY idx_dmv_violation_citizen (citizenid)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS dmv_exam_questions (
  ...
  active TINYINT(1) NOT NULL DEFAULT 1,
  PRIMARY KEY (id),
  KEY idx_dmv_exam_type (license_type, active)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS dmv_exam_attempts (
  ...
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  KEY idx_dmv_attempt_citizen (citizenid, created_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS dmv_vehicle_ledger (
  ...
  updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  UNIQUE KEY uq_dmv_vehicle_vin (vin),
  UNIQUE KEY uq_dmv_temp_plate (temporary_plate),
  UNIQUE KEY uq_dmv_assigned_plate (assigned_plate),
  KEY idx_dmv_vehicle_owner (buyer_citizenid, state),
  KEY idx_dmv_vehicle_current_plate (current_plate)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
-- Optional compatibility columns for existing DMV ledger deployments:
-- ALTER TABLE dmv_vehicle_ledger ADD COLUMN assigned_plate VARCHAR(16) NULL;
-- ALTER TABLE dmv_vehicle_ledger ADD COLUMN vehicle_name VARCHAR(160) NULL;
-- ALTER TABLE dmv_vehicle_ledger ADD COLUMN plate_installed_at TIMESTAMP NULL;
